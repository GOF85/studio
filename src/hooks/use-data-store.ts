        ...
        try {
            ...
            const allData = {
                ...
                proveedores: (proveedores || []).map((p: any) => ({
                    ...
                    ...p.contacto, // Spread the JSONB fields
                    // Ensure required fields have defaults if missing in JSONB
                    nombreEmpresa: p.contacto?.nombreEmpresa || p.nombre,
                    cif: p.contacto?.cif || '',
                    direccionFacturacion: p.contacto?.direccionFacturacion || '',
                    codigoPostal: p.contacto?.codigoPostal || '',
                    ciudad: p.contacto?.ciudad || '',
                    provincia: p.contacto?.provincia || '',
                    pais: p.contacto?.pais || 'España',
                    emailContacto: p.contacto?.emailContacto || '',
                    telefonoContacto: p.contacto?.telefonoContacto || '',
                    tipos: p.contacto?.tipos || []
                })),
