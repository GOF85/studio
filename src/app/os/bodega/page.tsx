        ...
        <Table>
          ...
          <TableBody>
            {orders.length > 0 ? orders.map(order => (
              <TableRow key={order.id} className={order.status === 'Asignado' ? "cursor-pointer" : ""} onClick={() => order.status === 'Asignado' && router.push(`/pedidos?osId=${osId}&type=Bodega&orderId=${order.id}`)}>
                <TableCell>
