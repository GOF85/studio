...
export default function AlmacenPage() {
  ...
  const handleEdit = (order: MaterialOrder) => {
    // This logic seems flawed as 'status' is not on MaterialOrder.
    // Assuming we check a similar field or remove the check.
    // For now, let's allow editing for demo purposes.
                ...
                <Table>
                    ...
                    <TableBody>
                        {materialOrders.length > 0 ? materialOrders.map(order => (
                            ...
                            <TableRow><TableCell colSpan={5} className="h-20 text-center text-muted-foreground">No hay pedidos de almacén para este servicio.</TableCell></TableRow>
                        )}
                    </TableBody>
                </Table>
            </div>
