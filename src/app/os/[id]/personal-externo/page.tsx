...
export default function PersonalExternoPage() {
  ...
  const ajusteProveedorRef = useRef<HTMLButtonElement>(null);
  const ajusteConceptoRef = useRef<HTMLInputElement>(null);
  const ajusteImporteRef = useRef<HTMLInputElement>(null);
  ...
  const handleAddAjuste = () => {
    const proveedorId = ajusteProveedorRef.current?.dataset.value;
    const concepto = ajusteConceptoRef.current?.value;
    const importe = ajusteImporteRef.current?.value;
    ...
    if (ajusteConceptoRef.current) ajusteConceptoRef.current.value = '';
    if (ajusteImporteRef.current) ajusteImporteRef.current.value = '';
  };
                        ...
                        <div className="space-y-2">
                           ...
                           <div className="border rounded-md p-2 space-y-2">
                            ...
                            <div className="flex gap-2 pt-2">
                                <Select name="ajuste-proveedor">
                                    <SelectTrigger className="h-8 text-xs" ref={ajusteProveedorRef}><SelectValue placeholder="Proveedor..."/></SelectTrigger>
                                    <SelectContent>
                                        {uniqueTurnoProviders.map(p => <SelectItem key={p.id} value={p.id}>{p.label}</SelectItem>)}
                                    </SelectContent>
                                </Select>
                                <Input id="ajuste-concepto" ref={ajusteConceptoRef} placeholder="Concepto" className="h-8 text-xs flex-grow"/>
                                <Input id="ajuste-importe" ref={ajusteImporteRef} type="number" step="0.01" placeholder="Importe" className="text-right h-8 w-24 text-xs"/>
