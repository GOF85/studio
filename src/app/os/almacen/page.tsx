    ...
    pickingSheets.forEach(sheet => {
        ...
        sheet.items.forEach(item => {
            if (item.type === 'Almacen') {
                ...
                // Find the original material order and mark it as 'picked'
               ...
               <Card className="flex-1 bg-muted/30">
                  ...
                  <CardContent className="space-y-2 h-full overflow-y-auto">
                      {assignedOrders.map(order => (
                          <Button key={order.id} variant="secondary" className="w-full h-auto text-left" onClick={() => router.push(`/pedidos?osId=${osId}&type=Almacen&orderId=${order.id}`)}>
