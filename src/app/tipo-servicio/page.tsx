...
import { PlusCircle, MoreHorizontal, Pencil, Trash2, FileDown, FileUp, List, ArrowLeft } from 'lucide-react';
...
export default function TipoServicioPage() {
  ...
  const handleExportCSV = () => {
    if (items.length === 0) {
      toast({ variant: 'destructive', title: 'No hay datos', description: 'No hay tipos de servicio para exportar.' });
      return;
    }
    ...
    link.setAttribute('download', 'tipo-servicio.csv');
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    toast({ title: 'Exportación completada', description: 'El archivo tipo-servicio.csv se ha descargado.' });
  };
  ...
  const handleImportCSV = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse<any>(file, {
      ...
      complete: (results) => {
        ...
        const importedData: TipoServicio[] = results.data.map(item => ({
            id: item.id || Date.now().toString() + Math.random(),
            servicio: item.servicio || '',
        }));
        
        localStorage.setItem('tipoServicio', JSON.stringify(importedData));
  ...
  const handleDelete = () => {
    ...
    localStorage.setItem('tipoServicio', JSON.stringify(updatedData));
    setItems(updatedData);
    toast({ title: 'Servicio eliminado', description: 'El registro se ha eliminado correctamente.' });
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="border rounded-lg">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="p-2">Servicio</TableHead>
              ...
              {filteredItems.length > 0 ? (
                filteredItems.map(item => (
                  <TableRow key={item.id}>
                    <TableCell className="font-medium p-2">{item.servicio}</TableCell>
                    <TableCell className="text-right p-2">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => router.push(`/tipo-servicio/${item.id}`)}>
