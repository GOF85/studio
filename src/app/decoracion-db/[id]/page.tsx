...
import { z } from 'zod';
import { Loader2, FileDown, Flower2, X } from 'lucide-react';
...
export const decoracionFormSchema = z.object({
  id: z.string(),
  concepto: z.string().min(1, 'El concepto es obligatorio'),
  precio: z.coerce.number().min(0, 'El precio debe ser positivo'),
});

type DecoracionFormValues = z.infer<typeof decoracionFormSchema>;

const defaultValues: Partial<DecoracionFormValues> = {
    concepto: '',
    precio: 0,
};

export default function DecoracionFormPage() {
  ...
  const form = useForm<DecoracionFormValues>({
    resolver: zodResolver(decoracionFormSchema),
    defaultValues,
  });

  useEffect(() => {
    if (isEditing) {
      const items = JSON.parse(localStorage.getItem('decoracionDB') || '[]') as DecoracionDBItem[];
  ...
  function onSubmit(data: DecoracionFormValues) {
    setIsLoading(true);

    let allItems = JSON.parse(localStorage.getItem('decoracionDB') || '[]') as DecoracionDBItem[];
    let message = '';
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
                ...
                <h1 className="text-3xl font-headline font-bold">{isEditing ? 'Editar' : 'Nuevo'} Concepto de Decoración</h1>
            </div>
          <div className="flex gap-2">
            <Button variant="outline" type="button" onClick={() => router.push('/decoracion-db')}>
                <X className="mr-2 h-4 w-4" />
                Cancelar
            </Button>
            <Button type="submit" form="decoracion-form" disabled={isLoading}>
        ...
        <Form {...form}>
          <form id="decoracion-form" onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
            <Card>
              <CardHeader>
