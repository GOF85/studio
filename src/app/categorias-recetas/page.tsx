...
import { PlusCircle, MoreHorizontal, Pencil, Trash2, FileDown, FileUp, BookText, ArrowLeft } from 'lucide-react';
...
export default function CategoriasRecetasPage() {
  ...
  const handleExportCSV = () => {
    if (items.length === 0) {
      toast({ variant: 'destructive', title: 'No hay datos', description: 'No hay categorías para exportar.' });
      return;
    }
    ...
    link.setAttribute('download', 'categorias-recetas.csv');
  ...
  const handleImportCSV = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse<any>(file, {
      ...
      complete: (results) => {
        ...
        const importedData: CategoriaReceta[] = results.data.map(item => ({
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
            <div>
                ...
                <h1 className="text-3xl font-headline font-bold flex items-center gap-3"><BookText />Categorías de Recetas</h1>
            </div>
          <div className="flex gap-2">
            <Button asChild>
              <Link href="/categorias-recetas/nuevo">
                    ...
                    <TableCell className="text-right p-2">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => router.push(`/categorias-recetas/${item.id}`)}>
