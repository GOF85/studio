...
export default function GastronomiaFormPage() {
  ...
  useEffect(() => {
    if (isEditing) {
      const gastronomia = JSON.parse(localStorage.getItem('gastronomia') || '[]') as Gastronomia[];
  ...
  function onSubmit(data: GastronomiaFormValues) {
    ...
    let allItems = JSON.parse(localStorage.getItem('gastronomia') || '[]') as Gastronomia[];
    let message = '';
    ...
    localStorage.setItem('gastronomia', JSON.stringify(allItems));
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex gap-2">
            <Button variant="outline" onClick={() => router.push('/gastronomia')}>Volver al listado</Button>
