...
import type { PedidoPartner, PedidoEntrega, ProductoVenta, Entrega, Proveedor, PersonalExterno, SolicitudPersonalCPR, AsignacionPersonal, EstadoPersonalExterno, EstadoSolicitudPersonalCPR, ComercialBriefingItem, Personal, PersonalExternoDB } from '@/types';
    ...
    useEffect(() => {
        if(isOpen) {
            ...
            const combined = isCprRequest
              ? allPersonalInterno.map(p => ({ label: `${p.nombre} ${p.apellidos}`, value: `${p.nombre} ${p.apellidos}`, id: p.id }))
              : allPersonalExterno
