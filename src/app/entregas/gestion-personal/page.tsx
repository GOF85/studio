...
import type { Entrega, PersonalEntrega, CategoriaPersonal, Proveedor, PersonalEntregaTurno, PedidoEntrega, EntregaHito } from '@/types';
                                ...
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                                            ...
                                            <TableHead className="py-2 px-3 w-[40%]">Observaciones</TableHead>
                                    ...
                                    <TableBody>
                                        {hitosConPersonal.map(hito => (
                                            <TableRow key={hito.id}>
                                                ...
                                                <TableCell className="py-2 px-3">{format(new Date(hito.fecha), 'dd/MM/yy')}</TableCell>
                           ...
                           <Table>
                                <TableHeader>
                                    ...
                                    <TableRow>
                                        ...
                                        <TableHead className="border-r px-1 py-2 bg-muted/30 w-24">Horas</TableHead>
                                        <TableHead className="px-2 py-2"></TableHead>
                                    </TableRow>
                                </TableHeader>
                                <TableBody>
                                        ...
                                        fields.map((field, index) => (
                                             <TableRow key={field.id}>
                                                ...
                                                <TableCell className="px-1 py-1">
                                                    <FormField control={control} name={`turnos.${index}.cantidad`} render={({ field: f }) => <FormItem><FormControl><Input type="number" min="1" {...f} onChange={(e) => { f.onChange(parseInt(e.target.value) || 1); setForceRecalc(c => c+1); }} className="w-16 h-9 text-center"/></FormControl></FormItem>} />
                                                </TableCell>
                                                <TableCell className="px-2 py-1">
                                                    <FormField
                                                        control={control}
                                                        ...
                                                        render={({ field: f }) => (
                                                            <FormItem>
                                                                <Select onValueChange={(val) => {f.onChange(val); setForceRecalc(c => c+1)}} value={f.value}>
                                                ...
                                                <TableCell className="border-l px-1 py-1 bg-muted/30">
                                                    <FormField control={control} name={`turnos.${index}.horaEntrada`} render={({ field: f }) => <FormItem><FormControl><Input type="time" {...f} onChange={(e) => { f.onChange(e.target.value); setForceRecalc(c => c+1);}} className="w-24 h-9" /></FormControl></FormItem>} />
                                                </TableCell>
                                                <TableCell className="px-1 py-1 bg-muted/30">
                                                    <FormField control={control} name={`turnos.${index}.horaSalida`} render={({ field: f }) => <FormItem><FormControl><Input type="time" {...f} onChange={(e) => { f.onChange(e.target.value); setForceRecalc(c => c+1);}} className="w-24 h-9" /></FormControl></FormItem>} />
                                                </TableCell>
                                                <TableCell className="border-r px-1 py-1 bg-muted/30">
                                                     <Input value={calculateHours(watchedFields[index]?.horaEntrada, watchedFields[index]?.horaSalida).toFixed(2)} className="w-24 h-9" readOnly />
                                                </TableCell>
