...
import type { ServiceOrder, Vertical } from '@/types';
  ...
  return (
    <main className="container mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-headline font-bold flex items-center gap-3"><Package />Previsión de Entregas</h1>
        <Button asChild>
          <Link href="/os?vertical=Entregas"> {/* Pass vertical type to new OS form */}
            ...
            {filteredAndSortedOrders.length > 0 ? (
              filteredAndSortedOrders.map(os => (
                <TableRow key={os.id} onClick={() => router.push(`/os?id=${os.id}`)} className="cursor-pointer">
                  ...
                  <TableCell>{os.spaceAddress || os.space}</TableCell>
              ...
              <TableRow>
                <TableCell colSpan={6} className="h-24 text-center">
                  No hay pedidos de entrega que coincidan con la búsqueda.
                </TableCell>
              </TableRow>
            )}
          </TableBody>
