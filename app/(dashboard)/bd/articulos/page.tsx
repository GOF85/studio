...
import { Pencil, Trash2, PlusCircle, Menu, FileUp, FileDown, Package, Search, Check, X, RefreshCw } from 'lucide-react';
import { downloadCSVTemplate, cn, normalizeCategoria } from '@/lib/utils';

import { useArticulosInfinite, useDeleteArticulo, useUpsertArticulo, useBulkDeleteArticulos, useSyncArticulosWithERP, useArticulosCategorias } from '@/hooks/use-data-queries';
...
function ArticulosPageContent() {
  ...
  const handleExportCSV = () => {
    const micecateringItems = items.filter(item => item.tipoArticulo === 'micecatering');
          ...
          <div ref={sentinelRef} className="h-24 flex items-center justify-center p-4 bg-muted/5 border-t border-border/40">
            {isFetchingNextPage && (
              <div className="flex items-center gap-3 px-6 py-3 rounded-full bg-background/80 backdrop-blur-md border border-border/40 shadow-xl">
                ...
                <span className="text-xs font-black uppercase tracking-widest text-primary">Cargando más artículos...</span>
              </div>
            )}
          </div>
        )}
