...
import { FileText, Plus, Search, Trash2, Edit2, ArrowLeft, Loader2, Package, PlusCircle, Pencil } from 'lucide-react';
...
export default function PlantillasPedidosPage() {
  const { data: plantillas, isLoading } = usePlantillasPedidos();
        ...
        <DialogContent className="rounded-[2rem] border-border/40 shadow-2xl max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-4 border border-border/40 rounded-[1.5rem] p-6 bg-muted/20">
              ...
              <div className="border border-border/40 rounded-xl bg-background/40 overflow-hidden">
                <Table>
                  <TableHeader className="bg-muted/30">
                    <TableRow className="hover:bg-transparent border-border/40">
                      <TableHead className="font-black text-[10px] uppercase tracking-wider text-muted-foreground/50">Artículo</TableHead>
                      <TableHead className="w-20 text-center font-black text-[10px] uppercase tracking-wider text-muted-foreground/50">Cant.</TableHead>
                  ...
                  <TableBody>
                    {items.length === 0 ? (
                      <TableRow>
                        <TableCell colSpan={3} className="text-center py-8 text-muted-foreground/40">
                          <Package className="h-8 w-8 mx-auto mb-2 opacity-10" />
                          <p className="text-[10px] font-black uppercase tracking-widest">No hay artículos</p>
                        </TableCell>
                      </TableRow>
                    ) : (
                      items.map((item) => (
                        <TableRow key={item.itemCode} className="border-border/40 hover:bg-primary/[0.02]">
                          <TableCell className="py-3">
                            <div className="font-bold text-sm">{item.description}</div>
                            <div className="text-[10px] text-muted-foreground uppercase tracking-wider">{item.itemCode}</div>
                          </TableCell>
                          <TableCell className="text-center font-black text-primary">x{item.quantity}</TableCell>
                          <TableCell>
                            <Button 
                              variant="ghost" 
                              size="icon" 
          ...
          <DialogFooter className="gap-2">
            ...
            <Button 
              ...
              className="rounded-xl h-12 font-black bg-primary hover:bg-primary/90 text-white px-8"
            >
