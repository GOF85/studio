    ...
    const handleSave = async (data: ElaborationFormValues, costePorUnidad: number) => {
        ...
        try {
            ...
            const dbData = {
                ...
                fotos_produccion_urls: data.fotos?.map(f => ({ value: f.url })) || [],
            ...
            // Insert elaboration into Supabase
            const { data: newElab, error } = await supabase
                .from('elaboraciones')
                .insert([dbData])
                .select()
                .single();
            ...
            if (newElab && data.componentes && data.componentes.length > 0) {
                const componentesToInsert = data.componentes.map(c => ({
                    elaboracion_padre_id: newElab.id,
                ...
                const { error: compError } = await supabase
                    ...
                    .insert(componentesToInsert);

                if (compError) throw compError;
            }

            // Construct the full object to return
            const savedElaboracion: Elaboracion = {
                ...data,
                id: newElab.id,
                costePorUnidad
            };
            ...
            console.error('Error saving elaboration:', e);
            toast({ variant: 'destructive', title: 'Error', description: 'No se pudo guardar la elaboración.' });
            setIsSubmitting(false);
        }
    };
                ...
                <div className="max-h-[70vh] overflow-y-auto -mx-6 px-6">
                    <ElaborationForm
                        initialData={{
                            ...
                            instruccionesPreparacion: '', videoProduccionURL: '', fotos: [],
                        }}
                        onSave={handleSave}
...
function RecetaFormPage() {
    ...
    const recalculateCostsAndAllergens = useCallback(async () => {
        ...
        const [
            ...
            { data: erpData },
            { data: elaboracionesData },
            { data: elaboracionComponentesData }
        ] = await Promise.all([
        ...
        // Map Ingredients
        const storedInternos: IngredienteInterno[] = (ingredientesData || []).map((i: any) => ({
            id: i.id,
        ...
        const storedErp: ArticuloERP[] = (erpData || []).map((a: any) => ({
            ...
            familiaCategoria: a.familia_categoria,
            precioCompra: a.precio_compra,
            descuento: a.descuento,
            unidadConversion: a.unidad_conversion,
            precio: a.precio,
            precioAlquiler: a.precio_alquiler,
            unidad: a.unidad_medida || 'UD',
            tipo: a.tipo,
            categoriaMice: a.categoria_mice,
            alquiler: a.alquiler,
        ...
        const allElaboraciones: Elaboracion[] = (elaboracionesData || []).map((e: any) => ({
            ...
            componentes: (elaboracionComponentesData || [])
    ...
    const { costeMateriaPrima, alergenos, partidasProduccion } = useMemo(() => {
        ...
        (watchedElaboraciones || []).forEach(elab => {
            ...
            (elabData?.alergenos || []).forEach((a: Alergeno) => allAlergenos.add(a));
