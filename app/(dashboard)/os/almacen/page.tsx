...
export default function AlmacenPage() {
   ...
   const { allItems, blockedOrders, pendingItems, itemsByStatus, totalValoracionPendiente } = useMemo(() => {
    ...
    const totalValoracionPendiente = pending.reduce((acc, item) => acc + ((((item.price || 0) || 0) || 0) * item.quantity), 0);
  ...
  const renderStatusModal = (status: StatusColumn) => {
    ...
    const all = [...itemsByStatus.Asignado, ...itemsByStatus['En Preparación'], ...itemsByStatus.Listo];
     const totalValue = all.reduce((sum, item) => sum + ((((item.price || 0) || 0) || 0) * item.quantity), 0);
    return (
      <DialogContent className="max-w-4xl">
              ...
              {all.map((item, index) => {
                ...
                return (
                  <TableRow key={`${item.itemCode}-${index}`}>
                    ...
                    <TableCell>{formatCurrency(item.quantity * (((item.price || 0) || 0) || 0))}</TableCell>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8">
            {(Object.keys(itemsByStatus) as StatusColumn[]).map(status => {
                const items = itemsByStatus[status];
                const totalValue = items.reduce((sum, item) => sum + ((((item.price || 0) || 0) || 0) * item.quantity), 0);
                return (
                <StatusCard 
                    key={status}
