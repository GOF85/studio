...
import { PlusCircle, Eye, Loader2, Trash2, FileText, ArrowLeft, Boxes, Clock, CheckCircle2 } from 'lucide-react'
...
export default function BioPage() {
  ...
  const renderSummaryModal = () => {
    ...
    return (
      <DialogContent className="max-w-4xl bg-background/95 backdrop-blur-md border-border/40">
        <DialogHeader>
          <DialogTitle className="text-[14px] font-black uppercase tracking-widest">Resumen de Artículos de Bio</DialogTitle>
        </DialogHeader>
  ...
  return (
    <Dialog open={!!activeModal} onOpenChange={(open) => !open && setActiveModal(null)}>
      ...
      <div className="sticky top-12 z-30 bg-background/60 backdrop-blur-md border-b border-border/40 mb-6">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Dialog>
              <DialogTrigger asChild>
                <Button variant="outline" size="sm" disabled={allItems.length === 0} className="text-[10px] font-black uppercase tracking-widest h-9 px-4">
        ...
        <div className="flex items-center justify-between p-4 border-b border-border/40 bg-blue-500/5">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right">
              <p className="text-[14px] font-black tracking-tight text-blue-600">{formatCurrency(totalValoracionPendiente)}</p>
              <p className="text-[9px] text-muted-foreground font-bold uppercase tracking-widest">Valoración total pendiente</p>
            </div>
          </div>
        </div>
            ...
            <Table>
              <TableHeader className="bg-muted/30">
                <TableRow className="hover:bg-transparent border-border/40">
                  ...
                  <TableHead className="h-10 px-4 text-[10px] font-black uppercase tracking-wider text-muted-foreground w-32">Cantidad</TableHead>
                  <TableHead className="h-10 px-4 text-[10px] font-black uppercase tracking-wider text-muted-foreground">Valoración</TableHead>
              ...
              <TableBody>
                {pendingItems.length > 0 ? (
                  pendingItems
                    ...
                    .map((item) => (
                      <TableRow key={item.itemCode + item.orderId} className="border-border/40 hover:bg-muted/10 transition-colors">
                        <TableCell className="px-4 py-2 text-[11px] font-medium">{item.description}</TableCell>
                        <TableCell className="px-4 py-2">
                          <Select
                            value={item.solicita}
                          ...
                          >
                            <SelectTrigger className="h-7 text-[10px] font-bold uppercase tracking-widest bg-background/50">
                              <SelectValue />
                            </SelectTrigger>
                            <SelectContent>
                              <SelectItem value="Sala" className="text-[10px] font-bold uppercase">Sala</SelectItem>
                              <SelectItem value="Cocina" className="text-[10px] font-bold uppercase">Cocina</SelectItem>
                            </SelectContent>
                          </Select>
                        </TableCell>
                        <TableCell className="px-4 py-2">
                        ...
                        <TableCell className="px-4 py-2">
                          <Input
                            ...
                            className="h-7 text-[11px] font-black bg-background/50"
                          />
                        </TableCell>
      ...
      <Card className="bg-background/60 backdrop-blur-md border-border/40 overflow-hidden">
        <div className="flex items-center justify-between p-4 border-b border-border/40 bg-amber-500/5">
          <div className="flex items-center gap-3">
              ...
              <TableBody>
                {blockedOrders.length > 0 ? (
                  blockedOrders.map((order) => (
                    <TableRow key={order.sheetId} className="border-border/40 hover:bg-muted/10 transition-colors">
                      <TableCell className="px-4 py-2">
                        <Link
                          href={`/almacen/picking/${order.sheetId}`}
                          className="text-blue-600 hover:underline text-[11px] font-black"
                        >
                          <Badge variant="secondary" className="bg-blue-500/10 text-blue-700 border-blue-200 text-[10px] font-black uppercase tracking-widest">
                            {order.sheetId}
                          </Badge>
                        </Link>
                      </TableCell>
                      <TableCell className="px-4 py-2">
                        <Badge 
                          ...
                          className={cn(
                            ...
                            order.status === 'Listo' ? 'border-emerald-500 text-emerald-600 bg-emerald-50' : 'border-amber-500 text-amber-600 bg-amber-50'
                          )}
                        >
                          {order.status}
                        </Badge>
